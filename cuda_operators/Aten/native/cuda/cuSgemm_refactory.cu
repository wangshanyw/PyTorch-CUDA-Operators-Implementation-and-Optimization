...
__global__ void gemm_kernel_cuda(
                ...
                float           *C, int ldc)
{
    ...
    void (*gpuSgemm) (const float *, const float *, float4 *, const int, const int, const int, const float, const float) = gemm_kernel_cuda;
    int BM = 128, BN = 128, TM = 8, TN = 8;
    int tmp_M = m + (128 - m % 128);
    int tmp_N = n + (128 - n % 128);
    int tmp_K = k + (128 - k % 128);
    dim3 blockDim(BN / TN, BM / TM), gridDim((n + BN - 1) / BN, (m + BM - 1) / BM);
    float4* c;
    gpuSgemm<<<gridDim, blockDim>>>(A, B, c, tmp_M, tmp_N, tmp_K, *alpha, *beta);
